...
class ServerViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        method="get",
        operation_summary="Returns all files and folders that are on the server along specified path",
        manual_parameters=[
            openapi.Parameter(
                "directory",
                openapi.IN_QUERY,
    ...
    def share(request):
        ...
        if directory.startswith(settings.SHARE_ROOT) and os.path.isdir(directory):
            ...
            return Response(
                "{} is an invalid directory".format(param), status=status.HTTP_400_BAD_REQUEST
            )
    ...
    @swagger_auto_schema(
        method="get",
        operation_summary="Method provides allowed plugins.",
    ...
    decorator=swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                "without_tasks",
                openapi.IN_QUERY,
                description="Returns only projects entities without related tasks",
                type=openapi.TYPE_BOOLEAN,
            ),
        ],
    ),
...
@method_decorator(
    name="create", decorator=swagger_auto_schema(operation_summary="Method creates a new project")
)
@method_decorator(
    name="retrieve",
    decorator=swagger_auto_schema(
        operation_summary="Method returns details of a specific project"
    ),
)
@method_decorator(
    name="destroy",
...
class ProjectViewSet(auth.ProjectGetQuerySetMixin, viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        def validate_project_limit(owner):
            ...
            if (
                not is_admin
                and settings.RESTRICTIONS["project_limit"] is not None
                and Project.objects.filter(owner=owner).count()
                >= settings.RESTRICTIONS["project_limit"]
            ):
    ...
    @swagger_auto_schema(
        method="get",
        operation_summary="Export project as a dataset in a specific format",
        manual_parameters=[
            openapi.Parameter(
                "format",
                openapi.IN_QUERY,
        ...
        responses={
            "202": openapi.Response(description="Exporting has been started"),
    ...
    @swagger_auto_schema(
        method="get",
        operation_summary="Method allows to download project annotations",
        manual_parameters=[
            openapi.Parameter(
                "format",
                openapi.IN_QUERY,
...
@method_decorator(
    name="update", decorator=swagger_auto_schema(operation_summary="Method updates a task by id")
)
@method_decorator(
    name="destroy",
    decorator=swagger_auto_schema(
...
class TaskViewSet(auth.TaskGetQuerySetMixin, viewsets.ModelViewSet):
    ...
    def _validate_task_limit(self, owner):
        ...
        if (
            not is_admin
            and settings.RESTRICTIONS["task_limit"] is not None
            and Task.objects.filter(owner=owner).count() >= settings.RESTRICTIONS["task_limit"]
        ):
            ...
            if not rq_job:
                ...
                if rq_job.is_finished:
                    ...
                    if exc_info.startswith(import_error_prefix):
                        ...
                        return Response(
                            data=exc_info, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                        )
                    ...
                    if rq_job.is_finished:
                        ...
                        if action == "download" and osp.exists(file_path):
                            ...
                            timestamp = datetime.strftime(
                                last_task_update_time, "%Y_%m_%d_%H_%M_%S"
                            )
                            filename = "task_{}_backup_{}{}".format(
                                db_task.name, timestamp, osp.splitext(file_path)[1]
                            )
                            return sendfile(
                                request,
                                file_path,
    ...
    @swagger_auto_schema(
        method="post",
        operation_summary="Method permanently attaches images or video to a task",
            ...
            try:
                ...
                if data_type == "chunk":
                    ...
                    for i in image.related_files.all():
                        ...
                        return HttpResponse(
                            io.BytesIO(result.tobytes()), content_type="image/jpeg"
                        )
                    return Response(
                        data="No context image related to the frame",
    ...
    @swagger_auto_schema(
        method="delete", operation_summary="Method deletes all annotations for a specific task"
    )
    @action(
        detail=True,
    ...
    def status(self, request, pk):
        ...
        response = self._get_rq_response(
            queue="default", job_id="/api/{}/tasks/{}".format(request.version, pk)
        )
    ...
    @action(
        detail=True, methods=["GET"], serializer_class=DataMetaSerializer, url_path="data/meta"
    )
        ...
        frame_meta = [
            {
                ...
                "height": item.height,
                "name": item.path,
                "has_related_context": hasattr(item, "related_files")
                and item.related_files.exists(),
            }
            for item in media
        ]
...
from utils.track_updater import TrackUpdater
...
@method_decorator(
    name="update", decorator=swagger_auto_schema(operation_summary="Method updates a job by id")
)
@method_decorator(
    name="partial_update",
    decorator=swagger_auto_schema(
...
class JobViewSet(viewsets.GenericViewSet, mixins.RetrieveModelMixin, mixins.UpdateModelMixin):
    ...
    @swagger_auto_schema(
        method="get", operation_summary="Method returns annotations for a specific job"
    )
    @swagger_auto_schema(
        method="put",
    ...
    @swagger_auto_schema(
        method="delete", operation_summary="Method deletes all annotations for a specific job"
    )
    @action(
        detail=True,
        methods=["GET", "DELETE", "PUT", "PATCH"],
    ...
    def annotations(self, request, pk):
        if "track_updater" not in self.__dict__:
            self.track_updater = TrackUpdater()

        self.get_object()  # force to call check_object_permissions

        if request.method == "GET":
            # Get all annotations from task 'pk'
            data = dm.task.get_job_data(pk)

            self.track_updater.init_task(data, pk, slogger.task[pk])

            writeln("GET", "", str(pk), json.dumps(data, indent=2))
            ...
            if format_name:
                writeln("PUT", "", str(pk), json.dumps(request.data, indent=2))
            ...
            if action == "create":
                processed_data = self.track_updater.patch_create_supplement(
                    request.data, pk, slogger.task[pk]
                )
            elif action == "update":
                processed_data = self.track_updater.patch_update(
                    request.data, pk, slogger.task[pk]
                )
            elif action == "delete":
                processed_data = self.track_updater.patch_delete(
                    request.data, pk, slogger.task[pk]
                )
            else:
                raise NotImplementedError()

            serializer = LabeledDataSerializer(data=processed_data)
            if serializer.is_valid(raise_exception=True):
                try:
                    ...
                    if action == "create":
                        self.track_updater.patch_create_update_id(
                            serializer.data, pk, slogger.task[pk]
                        )
    ...
    @swagger_auto_schema(
        method="get",
        operation_summary="Method returns list of reviews for the job",
...
@method_decorator(
    name="create", decorator=swagger_auto_schema(operation_summary="Submit a review for a job")
)
@method_decorator(
    name="destroy",
    decorator=swagger_auto_schema(operation_summary="Method removes a review from a job"),
)
...
@method_decorator(
    name="destroy",
    decorator=swagger_auto_schema(operation_summary="Method removes an issue from a job"),
)
@method_decorator(
    name="partial_update",
    decorator=swagger_auto_schema(
...
class IssueViewSet(viewsets.GenericViewSet, mixins.DestroyModelMixin, mixins.UpdateModelMixin):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if (
            "resolver_id" in request.data
            and request.data["resolver_id"]
            and db_issue.resolver is None
        ):
            # resolve
            ...
            "resolver_id" in request.data
            and not request.data["resolver_id"]
            and db_issue.resolver is not None
        ):
            # reopen
...
@method_decorator(
    name="partial_update",
    decorator=swagger_auto_schema(operation_summary="Method updates chosen fields of a user"),
)
@method_decorator(
    name="destroy",
    decorator=swagger_auto_schema(
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                "owner", openapi.IN_QUERY, description="A resource owner", type=openapi.TYPE_STRING
            ),
            openapi.Parameter(
                "credentials_type",
                openapi.IN_QUERY,
...
@method_decorator(
    name="destroy",
    decorator=swagger_auto_schema(
        operation_summary="Method deletes a specific cloud storage", tags=["cloud storages"]
    ),
)
@method_decorator(
    name="partial_update",
        ...
        try:
            ...
            if file_status == Status.NOT_FOUND:
                raise FileNotFoundError(
                    errno.ENOENT,
                    "Not found on the cloud storage {}".format(db_storage.display_name),
                    manifest_path,
                )
                ...
                raise PermissionError(
                    ...
                    "Access to the file on the '{}' cloud storage is denied".format(
                        db_storage.display_name
                    ),
                    manifest_path,
                )

            full_manifest_path = os.path.join(db_storage.get_storage_dirname(), manifest_path)
            if not os.path.exists(full_manifest_path) or datetime.utcfromtimestamp(
                os.path.getmtime(full_manifest_path)
            ).replace(tzinfo=pytz.UTC) < storage.get_file_last_modified(manifest_path):
    ...
    def preview(self, request, pk):
        try:
            ...
            if not os.path.exists(db_storage.get_preview_path()):
                ...
                if not db_storage.manifests.count():
                    raise Exception(
                        "Cannot get the cloud storage preview. There is no manifest file"
                    )
                preview_path = None
                for manifest_model in db_storage.manifests.all():
                    ...
                    if not os.path.exists(full_manifest_path) or datetime.utcfromtimestamp(
                        os.path.getmtime(full_manifest_path)
                    ).replace(tzinfo=pytz.UTC) < storage.get_file_last_modified(
                        manifest_model.filename
                    ):
...
def _import_annotations(request, rq_id, rq_func, pk, format_name):
    format_desc = {f.DISPLAY_NAME: f for f in dm.views.get_import_formats()}.get(format_name)
    ...
    if not rq_job:
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            rq_job = queue.enqueue_call(
                func=rq_func, args=(pk, filename, format_name), job_id=rq_id
            )
...
def _export_annotations(db_instance, rq_id, request, format_name, action, callback, filename):
        ...
        if request_time is None or request_time < last_instance_update_time:
            ...
            if rq_job.is_finished:
                ...
                if action == "download" and osp.exists(file_path):
                    ...
                    return sendfile(
                        request, file_path, attachment=True, attachment_filename=filename.lower()
                    )
                else:
                    if osp.exists(file_path):
                        return Response(status=status.HTTP_201_CREATED)
    ...
    ttl = (
        dm.views.PROJECT_CACHE_TTL if isinstance(db_instance, Project) else dm.views.TASK_CACHE_TTL
    ).total_seconds()
    queue.enqueue_call(
        func=callback,
